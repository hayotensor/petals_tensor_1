...
class _AutoDistributedBase:
    ...
    def from_pretrained(cls, model_name_or_path: Union[str, os.PathLike, None], *args, **kwargs) -> PretrainedConfig:
        ...
        print("auto_config.py | proper_cls", proper_cls)
        if proper_cls is None:
            raise ValueError(f"Petals does not have {cls.__name__} for model type {config.model_type}")

        print("auto_config.py | proper_cls.from_pretrained(model_name_or_path, *args, **kwargs)", proper_cls.from_pretrained(model_name_or_path, *args, **kwargs))
...
class DefaultRevisionMixin:
    ...
    def from_pretrained(
        ...
        print("auto_config.py | cls", cls)
        print("auto_config.py | model_name_or_path", model_name_or_path)
        print("auto_config.py | args", args)
        print("auto_config.py | revision", revision)
        print("auto_config.py | kwargs", kwargs)
