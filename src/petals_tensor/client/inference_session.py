...
from petals_tensor.client.routing import RemoteSequenceManager, maybe_log_traceback
...
class InferenceSession:
    ...
    def __init__(self, sequence_manager: RemoteSequenceManager, max_length: int, peer_ids: Optional[List[str]] = None, peer_id: Optional[str] = None):
        ...
        print("InferenceSession init sequence_manager", sequence_manager)
    ...
    def step(
        ...
        print("inferece_session.py / step() 262")
        ...
        print("inferece_session.py / step() self.num_blocks", self.num_blocks)
    ...
    def _update_sequence(self, server_idx: int, block_idx: int, attempt_no: int) -> int:
        print("_update_sequence server_idx", server_idx)
        print("_update_sequence block_idx", block_idx)
        print("_update_sequence attempt_no", attempt_no)
        ...
        print("_update_sequence update_end", update_end)
        if attempt_no >= 1:
            logger.debug(
