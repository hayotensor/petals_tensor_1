...
from petals_tensor.client.routing.sequence_info import RemoteSequenceInfo
from petals_tensor.client.routing.spending_policy import NoSpendingPolicy
from petals_tensor.data_structures import ModuleUID, RemoteSpanInfo, ServerState
...
from petals_tensor.utils.random import sample_up_to
...
class RemoteSequenceManager:
    ...
    def make_sequence(
        ...
        *,
        ...
        print("make_sequence")
        print("make_sequence start_index", start_index)
        print("make_sequence end_index", end_index)
        ...
        print("make_sequence span_sequence", span_sequence)
    ...
    def _make_sequence_with_specific_peers(self, peer_ids: List[str], start_index: int, end_index: int) -> List[RemoteSpanInfo]:
        client_server_rtts = self.ping_aggregator.to_dict()
        ...
        while current_index < end_index:
            ...
            if any(span.peer_id in set(peer_ids) for span in candidate_spans):
                logger.info("client _make_sequence_with_specific_peers any()")
                peer = None
                for span in candidate_spans:
                    for peer_id in peer_ids:
                        if span.peer_id == peer_id:
                ...
                logger.info("client _make_sequence_with_specific_peers any() NOT FOUND")
                # We choose longer servers to minimize the number of hops but leave some randomization
                # to distribute the load. We also exclude servers known to be unreachable.
                eps = 1e-6
                span_weights = np.array(
