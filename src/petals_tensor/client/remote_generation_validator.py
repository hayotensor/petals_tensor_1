...
from petals_tensor.utils.misc import DUMMY, docstring_from
...
class RemoteGenerationMixinValidation(_SkipTokensMixin):
    """
    ...
    def generate(
        self, inputs: Optional[torch.Tensor] = None, peer_id: Optional[str] = None, *args, session: Optional[InferenceSession] = None, **kwargs
    ):
        self._fix_generate_kwargs(kwargs)
        print("remote_generation.py | generate 88")

        print("remote_generation.py | inputs", inputs)
        print("remote_generation.py | *args", str(args))
        print("remote_generation.py | session", session)
        print("remote_generation.py | **kwargs", str(kwargs))
    ...
    # @docstring_from(transformers.GenerationMixin.generate.__doc__)
    # def generate_with_specific_peer(
    #     self, peer_id: str, inputs: Optional[torch.Tensor] = None, *args, session: Optional[InferenceSession] = None, **kwargs
    # ):
    #     self._fix_generate_kwargs(kwargs)
    #     print("remote_generation.py | generate_with_specific_peer")

    #     print("remote_generation.py | inputs", inputs)
    #     print("remote_generation.py | peer_id", peer_id)
